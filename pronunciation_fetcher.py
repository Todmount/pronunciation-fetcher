...
def api_key_requirement(provider: str) -> bool:
    if provider in needs_api:
        log.info("API key is required for this source.")
        return True
    else:
        log.info("No API key required for this source.")
        return False
...
def word_input() -> str:
    ...
    log.debug(f"User provided words: {words}") # remember it's a really long str, not a list
    normalized_words, _ = normalize_words(words)
    log.debug(f"Normalized words: {normalized_words}")
    log.debug(f"Words after normalization: {len(normalized_words)}")
...
def save_failed_to_txt(output_folder: str, failed_words: list, provider: str) -> None:
    ...
    if choice:
        ...
        log.debug("User decided not export failed words to txt")
...
def get_words(failed_words: list[str] | None) -> list[str] | None:
    while True:
        if failed_words:
            ...
            log.debug(f"words_to_process in get_words: {words_to_process}")
